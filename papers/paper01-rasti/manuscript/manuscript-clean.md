...
The historical criterion contained no multi-scale conjunct because multi-scale parameterisation has an established design lineage. No current statement is made about whether any comparator meets that criterion: every Table 5 element cell remains `unknown` pending full-text recoding.
...
The discovery inventory remains updateable. A newly verified record may be added by identity, but an element cell changes from `unknown` only after a full-text evidence bundle records the relevant locator and adjudication. Both Table 5 and Appendix A must then be regenerated from the same bundle; unresolved or inaccessible records never enter a negative count.
...
Figure 2 presents the paper's two-layer framework–governance architecture. This section develops the framework layer; the governance components are specified in Section 6, and their empirical motivation is discussed in Section 8.4.
...
The inference stack is presented as **designed and partially evidenced**. Table 2 lists every component together with the tier of evidence that currently stands behind it, and no statement in this section exceeds the tier recorded there. Only one row — delayed acceptance, together with the diagnostics contract it exercises — carries Synthetic-run evidence; every other row is design.
...
Simulation-based calibration is specified as a validation instrument in Section 6 rather than as a sampler diagnostic here; its contract, including the treatment of ordered scalar ranks, categorical events and the sensitivity of conclusions to the choice of test quantity, follows Cook et al. (2006), Talts et al. (2018, arXiv:1804.06788, preprint) and Modrák et al. (2025).
...
Three of the four boundaries carry a mandatory co-disclosure or a mandatory negation, and they are discharged where the corresponding evidence is reported rather than here: the verbatim disclosure statements are in Sections 7.1 and 7.3 and are collected in Section 8.5. This section registers the obligation; it does not restate the statements, and it does not summarise them.
...
The formal 42-run re-pilot includes the s00–s04 × λ ∈ {0, 1, 10, 100, 1000, 10000} grid, uses four chains, and sets n_draws from the empirical R1 scaling slope.
...
For the analytic reason registered in preregistration Appendix E.1, `mode_visits` remains a mandatory run-level disclosure, including its operational fields, the surrogate rationale, and the analytic upper-bound-to-threshold ratio at the registered chain count.
...
**Table 4a. Conservative mapping from verified method components to the joint-graph interface.**
...
**Figure 5.** Convergence and effective-sample-size diagnostics for the algorithm-correctness study, together with frequentist same-truth coverage at nominal levels 0.90 and 0.95. Coverage counts are 358/400 and 378/400. Error bars are two-sided 95% Wilson score intervals without continuity correction: [0.861, 0.921] and [0.918, 0.963], respectively. This is not simulation-based calibration. Diagnostic thresholds are read from the bound contract at run time rather than redefined in the caption.
...
Two consequences of that disclosure are worth making explicit, because they are easy to lose. **A low residual is not a success criterion here** — it is the direction in which this particular asset is known to mislead, which is why the overfitting condition is recorded as a warning rather than as a gate. And **the magnetic model-recovery failure is a property of the retained record, not a resolved defect**: the later run does not overturn it, it operates under a claim boundary that never asserted recovery in the first place.
...
The Chinese-language audit records its queries, date, identity checks and unresolved items. It corrects the 2020 gravity–magnetic–MT record to Yan Zhengwen et al., 63(2):736–752, DOI 10.6038/cjg2020M0355, and verifies related multi-geophysical cross-gradient work. A purported 2016 three-dimensional Bayesian MT–seismic record could not be independently bound and remains `unresolved`; it is neither cited nor treated as negative evidence. This targeted audit reduces a metadata error but does not close the language-coverage gap.
...
Three production limitations remain. First, blinding of the joint synthetic experiment used hashes and separated roles within one operator rather than institutionally independent parties. Second, a synthesis-phase process-integrity event was detected when reported deliverables were absent on disk; those adjudications were voided and rebuilt from verified inputs. Detection demonstrates that one control fired, not that all remaining records are correct. Third, a public GitHub repository exists at `https://github.com/xiaopengcug/GeoDeepBayes1.0.1`, but the paper-specific Stage 4 materials have not been pushed and no immutable public revision or archive identifier is yet claimed. AI involvement is disclosed separately in the Declarations.
...
|---|---|---|
| Geology | No independently held field target or structural truth is bound | Frozen geological hypotheses, target definition and blind or spatio-temporal holdout |
...
**No AI output is presented as human authorship.** Every action attributed to a human in this paper corresponds to a dated decision record; no AI-generated content is attributed to a human author.
...
**Countermeasures in place.** The experimental design and failure boundary were pre-registered; generation and inference artefacts are hash-bound; failed runs are retained; and adversarial review occurred at three checkpoints. For literature comparison, unsupported ✓/◐/✗ scores and element counts have been withdrawn. Table 5 and Appendix A now expose `unknown` cells and the full-text evidence requirements for any future recoding rather than inviting readers to reproduce an under-supported score.
...
The framework is implemented in the `geodeepbayes` package. A public repository exists at `https://github.com/xiaopengcug/GeoDeepBayes1.0.1`, but the manuscript-specific Stage 4 revision, diagnostic replay and evidence bundle have not yet been pushed; no immutable public commit, tag or archive identifier is claimed here. The governance archive binds the diagnostic contract, evidence registry, per-evidence hashes, joint-pilot registration and failure records. The Supplement provides the observed Windows/Python environment, minimum input/output/unit contract, local test and replay commands, supported component boundaries and failure-escalation rules. A clean-environment independent replay has not been completed, and the reported environment is not a performance benchmark.

The manuscript reports Synthetic-run and data-ingestion evidence but no Field-validated result. Public-data source records and redistribution terms must travel with any released artefact; vendor DLLs and licence-bound SDKs are not distributed. The public GitHub remote is currently a discovery route rather than a citation-grade reproduction package for this paper because the paper-specific materials are not yet pushed and no immutable release identifier exists. Before submission, any released code, data manifests, replay scripts and expected statuses must be bound to an exact revision with licence and access statements; until then, external reproducibility is not claimed.
...
Every current Table 5 element cell is `unknown`. The former ✓/◐/✗ grades and their counts are historical working records only and carry no manuscript decision role.
...
The inventory separates bibliographic discovery from element adjudication. A non-`unknown` cell requires a full-text quotation or paraphrase with page, section or paragraph locator and an identity-verified source. No title, abstract, metadata record or inaccessible candidate is scored as absence. Rows may be recoded only by updating the evidence bundle and regenerating both Table 5 and this appendix; unresolved records remain outside any count.
...
Every entry cited in this paper passed the protocol described here, and no claim is made about literature outside the set it produced. The protocol is published in full because one of the paper's contributions is a source discipline, and a source discipline that cannot itself be audited is not one.
