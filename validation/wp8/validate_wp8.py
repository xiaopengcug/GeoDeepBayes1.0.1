...
# 2026-08-02 文档治理：wp8 随论文验证代码迁入治理档案；ROOT 为档案根，
# 仓库级 open-data 目录经 REPO_ROOT 访问。
REPO_ROOT = Path(__file__).resolve().parents[6]
WP8 = Path(__file__).resolve().parent
OPEN_DATA = REPO_ROOT / "_bmad-output/planning-artifacts/research/open-data"
...
def audit_synthetic_phase() -> dict[str, Any]:
    ...
    if method_contract_ok:
        ...
        if isinstance(file_hashes, dict):
            # 记录键 "uv.lock" 为仓库级锁定文件；与生成侧 _provenance_bindings
            # 的解析规则保持一致（其余键按档案根解析）。
            repo_level_files = {"uv.lock"}
            for relative, recorded_hash in file_hashes.items():
                ...
                resolve_base = (
                    REPO_ROOT if relative in repo_level_files else ROOT
                )
                candidate = (
                    (resolve_base / relative).resolve()
                    if safe_relative
                    else ROOT.parent
                )
                if (
                    not safe_relative
                    or resolve_base.resolve() not in candidate.parents
                ):
