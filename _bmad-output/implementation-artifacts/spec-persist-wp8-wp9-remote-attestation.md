...
- 首轮另列 162 个不重复执行/源码/测试控制输入，但远程 clean checkout 证明该集合漏掉 pytest 直接导入的 WP8 顶层脚本。计划现以浅层 glob 纳入 `validation/wp8/*.py` 全部 204 个脚本，共列 363 个控制输入；合计 396 个 `required-git` 路径，其中 239 个当前内容尚未进入 `707f4aab…` 基线。
...
- 首轮白名单暂存按 195 个 required 路径执行，119 个路径相对 HEAD 形成变更，白名单外 staged 路径为 0。远程失败后，builder 的基线更新为 `707f4aab…`，扩大后的 396 路径全部通过扫描且无 unresolved ignored 项；下一提交必须按新集合重新精确暂存。
