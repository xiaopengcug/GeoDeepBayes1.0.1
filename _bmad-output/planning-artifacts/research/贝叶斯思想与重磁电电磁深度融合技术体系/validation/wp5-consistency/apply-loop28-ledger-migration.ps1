...
param(
    ...
    [string]$MigrationPath = (Join-Path $PSScriptRoot 'claim-ledger.loop28.text-migration.json')
)

$ErrorActionPreference = 'Stop'
$root = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
function Fail([string]$Message) { throw "WP5 loop28 migration failed: $Message" }
...
if (@($migration.entries).Count -ne 42 -or [int]$migration.replacements -ne 28 -or [int]$migration.tombstones -ne 14) { Fail 'migration cardinality' }
...
foreach ($m in @($migration.entries)) {
    ...
    foreach ($property in $expectedNames) {
        ...
        if ($m.id -ceq 'CODE-DOC-06-0006' -and $property -in @('symbols','tests')) { continue }
...
if (@($next.entries).Count -ne 399 -or @($next.tombstones).Count -ne 649 -or (@($next.entries).Count + @($next.tombstones).Count) -ne 1048) { Fail 'output cardinality' }
